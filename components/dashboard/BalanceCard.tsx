...
import { ArrowDownLeft, ArrowLeftRight, ArrowUpRight, Plus, Zap } from 'lucide-react';
...
export function BalanceCard({ spentThisMonth }: { spentThisMonth: number }) {
  ...
  const actions = [
    ...
    { label: t('dashboard.request'), icon: ArrowDownLeft, href: '/transact/request' },
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl bg-lime p-6 shadow-card transition-shadow duration-200 hover:shadow-card-hover">
      ...
      <div className="relative">
        <p className="mb-1 text-sm font-medium text-black/50">{t('dashboard.walletBalance')}</p>
        <h2 className="font-number mb-1 text-4xl font-bold tracking-tight text-black">
          {t('common.rs')}. <CountUp end={balance} duration={1.2} separator="," decimals={balance % 1 ? 2 : 0} />
        </h2>
        <p className="text-sm text-black/60">
          {t('dashboard.accountBalance')}: {formatPKR(accountBalance)}
